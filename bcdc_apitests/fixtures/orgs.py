...
def org_create_if_not_exists_fixture(remote_api_super_admin_auth,
    ...
    deal wiht this condition, catch ValidationError, purge, then try to
    recreate
    '''
...
def org_setup_fixture(remote_api_super_admin_auth, test_session_organization,
    ...
    if not org_exists_fixture:
        ...
        org_data = remote_api_super_admin_auth.action.organization_show(id=session_test_org_data['name'])
        LOGGER.debug("org_data from show: %s", org_data)
    yield org_data
    # TODO: debugging comment out tear down
